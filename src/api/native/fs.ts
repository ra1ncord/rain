 ...
 */
export async function clearFolder(path: string, { prefix = "raincord/" } = {}) {
 ...
 */
export async function removeFile(path: string, { prefix = "raincord/" } = {}) {
 ...
 * @param path Path to the file
 */
export async function removeCacheFile(path: string, prefix = "raincord/") {
    if (typeof NativeFileModule.removeFile !== "function") throw new Error("'fs.removeFile' is not supported");
 ...
 * @param path Path to the file
 */
export async function fileExists(path: string, { prefix = "raincord/" } = {}) {
 ...
 */
export async function writeFile(path: string, data: string, { prefix = "raincord/" } = {}): Promise<void> {
 ...
 */
export async function readFile(path: string, { prefix = "raincord/", encoding = "utf8" }: { prefix?: string, encoding?: "utf8" | "base64" } = {}): Promise<string> {
    try {
 ...
 */
export async function downloadFile(url: string, path: string, { prefix = "raincord/" } = {}) {
