// Material Color Utilities - HCT Solver
...
export class HctSolver {
    ...
    private static nthVertex(y: number, n: number): number[] {
        ...
        if (n < 4) {
            ...
            if (HctSolver.isBounded(r)) return [r, g, b];
            return [-1.0, -1.0, -1.0];
        }
        if (n < 8) {
            const b = coordA;
            ...
            if (HctSolver.isBounded(g)) return [r, g, b];
            return [-1.0, -1.0, -1.0];
        }
        const r = coordA;
        const g = coordB;
        const b = (y - r * kR - g * kG) / kB;
        if (HctSolver.isBounded(b)) return [r, g, b];
        return [-1.0, -1.0, -1.0];
    }
    ...
    private static findResultByJ(hueRadians: number, chroma: number, y: number): number {
        ...
        for (let iterationRound = 0; iterationRound < 5; iterationRound++) {
            ...
            const ac = viewingConditions.aw * jNormalized ** (1.0 / viewingConditions.c / viewingConditions.z);
