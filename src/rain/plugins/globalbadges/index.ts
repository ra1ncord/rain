...
export default definePlugin({
  ...
  start() {
    ...
    after("default", useBadgesModule, ([user], result) => {
      const [customBadges, setCustomBadges] = useState<CustomBadges>({});
      
      useEffect(() => {
        if (!user) return;
        fetchBadges(user.userId).then(badges => {
          setCustomBadges(badges);
        });
      }, [user?.userId]);
      
      if (!user || Object.keys(customBadges).length === 0) return;
      
      if (customBadgesSettings.customs && customBadgesSettings.mods) return;
      
      Object.entries(customBadges).forEach(([key, value]: [string, any]) => {
        const isModBadge = ['aliu', 'bd', 'enmity', 'goosemod', 'replugged', 'vencord', 'equicord', 'raincord'].includes(key);
