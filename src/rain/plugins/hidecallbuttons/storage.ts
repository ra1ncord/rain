...
export async function waitForHideCallButtonsSettingsHydration(): Promise<void> {
    return new Promise((resolve) => {
        if (useHideCallButtonsSettings.getState()._hasHydrated) {
            resolve();
            return;
        }

        const unsubscribe = useHideCallButtonsSettings.subscribe((state) => {
